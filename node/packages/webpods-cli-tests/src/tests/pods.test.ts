...
describe("CLI Pod Commands", function () {
  ...
  describe("create command", () => {
    it("should create a new pod", async () => {
      ...
      expect(result.stdout).to.include("Access it at: https://test-pod.webpods.org");
      
      // Verify pod was created in database
      const pod = await testDb.getDb().oneOrNone(
        "SELECT * FROM pod WHERE name = $1",
        ["test-pod"]
      );
  ...
  describe("delete command", () => {
    ...
    it("should only allow owner to delete pod", async () => {
      // Create another user
      const otherUser = await testDb.getDb().one(
