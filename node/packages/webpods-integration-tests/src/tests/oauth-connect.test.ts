...
describe("OAuth Connect Endpoint", () => {
  ...
  describe("GET /connect", () => {
    it("should redirect to Hydra with client parameters", async () => {
      ...
      expect(location).to.include("redirect_uri=https%3A%2F%2Ftestapp.com%2Fcallback");
    ...
    it("should return 404 for unknown client", async () => {
      const response = await client.get("/connect?client_id=unknown-client-123");
    ...
    it("should only be available on main domain", async () => {
      ...
      const response = await podClient.get(`/connect?client_id=${testClientId}`);
