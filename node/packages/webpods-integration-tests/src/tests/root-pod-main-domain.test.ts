...
describe('WebPods Root Pod Main Domain', () => {
  ...
  before(async () => {
    ...
    const homeResponse = await rootClient.post('/site/home', '<h1>Welcome to Main Domain</h1>');
    if (homeResponse.status !== 201) {
      throw new Error(`Failed to create home page: ${homeResponse.status} ${JSON.stringify(homeResponse.data)}`);
    }
    
    const aboutResponse = await rootClient.post('/site/about', '<h1>About Page</h1>');
    if (aboutResponse.status !== 201) {
      throw new Error(`Failed to create about page: ${aboutResponse.status} ${JSON.stringify(aboutResponse.data)}`);
    }
    
    const statusResponse = await rootClient.post('/api/status', JSON.stringify({ status: 'ok', version: '2.0' }));
    if (statusResponse.status !== 201) {
      throw new Error(`Failed to create status: ${statusResponse.status} ${JSON.stringify(statusResponse.data)}`);
    }
    
    // Configure links for clean URLs
    const linksResponse = await rootClient.post('/.meta/links', {
      '/': 'site/home',
      '/about': 'site/about',
      '/status': 'api/status'
    });
    if (linksResponse.status !== 201) {
      throw new Error(`Failed to create links: ${linksResponse.status} ${JSON.stringify(linksResponse.data)}`);
    }
  ...
  describe('Authenticated operations via main domain', () => {
    it('should allow POST to root pod via main domain with auth', async () => {
      ...
      const response = await mainClient.post('/dynamic/content', 'Dynamic data');
    ...
    it('should enforce permissions on main domain', async () => {
      ...
      mainClient.setAuthToken(authToken);
      const createResponse = await mainClient.post('/secured/data?access=private', 'Secret');
