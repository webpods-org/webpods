...
describe('Session Expiry and Cleanup', () => {
  ...
  describe('PKCE State Expiry', () => {
    ...
    it('should enforce 10-minute TTL for PKCE states', async () => {
      ...
      const ttlMinutes = (withinTTL!.expires_at.getTime() - now.getTime()) / 60000;
  ...
  describe('Cleanup Job Behavior', () => {
    ...
    it('should only delete expired records', async () => {
      ...
      await db('oauth_state').insert([
        { state: 'st1', code_verifier: 'v1', expires_at: new Date(now.getTime() - 1000) },
        { state: 'st2', code_verifier: 'v2', expires_at: new Date(now.getTime() + 1000) },
        { state: 'st3', code_verifier: 'v3', expires_at: new Date(now.getTime() - 2000) }
      ]);
