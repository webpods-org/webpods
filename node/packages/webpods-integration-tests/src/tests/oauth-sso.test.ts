...
describe('Full SSO OAuth Flow', () => {
  ...
  it('should complete full OAuth flow with mock provider', async () => {
    ...
    // Parse session data
    const sessionData = typeof sessions[0].sess === 'string' 
  ...
  it('should handle session logout and require re-authentication', async () => {
    ...
    const sessionsAfterLogout = await testDb.getDb()('session')
      .where('sid', sessionId)
      .select('*');
  ...
  it('should handle pod-specific OAuth flow', async () => {
    ...
    const podAuthResponse = await client.get('/auth/authorize?pod=alice&redirect=/admin', {
      followRedirect: false
    });
    ...
    const stateData = await testDb.getDb()('oauth_state')
      .where('state', state)
      .first();
