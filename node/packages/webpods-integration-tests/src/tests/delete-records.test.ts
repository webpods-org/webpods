...
describe('WebPods Record Deletion', () => {
  ...
  beforeEach(async () => {
    ...
    const [owner] = await db('user').insert({
  ...
  describe('Stream vs Record Deletion', () => {
    ...
    it('should prevent deletion of system streams', async () => {
      ...
      expect(response.status).to.equal(403);
      expect(response.data.error.message).to.include('System streams cannot be deleted');
    });
  });
  ...
  describe('Error Messages', () => {
    it('should provide clear error when neither stream nor record exists', async () => {
      ...
      expect(response.data.error.message).to.include("no stream 'foo/bar' with record 'baz'");
    });
  });
});