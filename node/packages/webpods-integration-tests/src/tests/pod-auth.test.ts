...
describe('Pod-Specific Authentication with SSO', () => {
  ...
  function createGlobalToken(userId: string, authId: string, email: string = 'test@example.com') {
    return jwt.sign(
      {
        user_id: userId,
        auth_id: authId,
  ...
  describe('Pod Login Flow', () => {
    ...
    it('should include redirect parameter in login flow', async () => {
      ...
      expect(response.status).to.equal(302);
      expect(response.headers.location).to.include(`redirect=${encodeURIComponent('/dashboard')}`);
    });
  });
