...
export async function streams(options: StreamsOptions): Promise<void> {
  ...
  try {
    ...
    if (!result.success) {
      ...
      logger.error("Stream listing failed", { pod: options.pod, error: result.error });
      process.exit(1);
    }
    ...
    switch (format) {
      ...
      default: // table
        ...
        output.print("─".repeat(30));
        output.print(`Total: ${streamList.length} stream${streamList.length === 1 ? "" : "s"}`);
    }
    
    logger.info("Streams listed successfully", { pod: options.pod, count: streamList.length, format });
  } catch (error: any) {
    logger.error("Streams command failed", { pod: options.pod, error: error.message });
...
export async function deleteStream(options: DeleteStreamOptions): Promise<void> {
  ...
  try {
    logger.debug("Deleting stream", { pod: options.pod, stream: options.stream, force: options.force });
    
    if (!options.force) {
      output.print(`WARNING: This will permanently delete stream '/${options.stream}' and ALL its records.`);
      output.print("This action cannot be undone!");
      output.print("Use --force to skip this confirmation.");
      logger.info("Stream deletion cancelled - confirmation required", { pod: options.pod, stream: options.stream });
      process.exit(0);
    }
    ...
    if (!result.success) {
      if (result.error.code === "NOT_FOUND") {
        output.error(`Stream '/${options.stream}' not found in pod '${options.pod}'.`);
        logger.warn("Stream not found for deletion", { pod: options.pod, stream: options.stream });
      } else {
        output.error("Error: " + result.error.message);
        logger.error("Stream deletion failed", { pod: options.pod, stream: options.stream, error: result.error });
      }
      process.exit(1);
    }
    
    output.success(`Stream '/${options.stream}' deleted successfully.`);
    logger.info("Stream deleted successfully", { pod: options.pod, stream: options.stream });
  } catch (error: any) {
    logger.error("Delete stream command failed", { pod: options.pod, stream: options.stream, error: error.message });
    output.error("Error: " + error.message);
    process.exit(1);
  }
}