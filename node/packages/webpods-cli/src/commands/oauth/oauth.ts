 ...
 */
export async function oauthRegister(options: OAuthRegisterOptions): Promise<void> {
  const output = createCliOutput(options.quiet);
  
  try {
    const requestedPods = options.pods ? options.pods.split(",").map(p => p.trim()) : [];
    logger.debug("Registering OAuth client", { name: options.name, redirect: options.redirect, requestedPods });
    
    const result = await apiRequest<{ client_id: string; client_secret: string }>("/api/oauth/clients", {
      method: "POST",
      body: {
    ...
    if (!result.success) {
      ...
      logger.error("OAuth client registration failed", { name: options.name, error: result.error });
      process.exit(1);
    }
    ...
    logger.info("OAuth client registered successfully", { name: options.name, clientId: result.data.client_id });
  } catch (error: any) {
    logger.error("OAuth register command failed", { name: options.name, error: error.message });
...
export async function oauthList(options: GlobalOptions): Promise<void> {
  ...
  try {
    ...
    if (clients.length === 0) {
      output.print("No OAuth clients found. Create one with 'pod oauth register'.");
      return;
    }
    ...
    switch (format) {
      ...
      case "yaml":
        ...
        break;
      case "csv":
        output.print("client_id,client_name,created_at,redirect_uris,requested_pods");
        clients.forEach(client => {
          output.print(`"${client.client_id}","${client.client_name}","${client.created_at}","${client.redirect_uris.join(";")}","${client.requested_pods.join(";")}"`);
        });
        break;
      default: // table
        ...
        clients.forEach(client => {
          ...
          output.print(`  Created: ${new Date(client.created_at).toLocaleDateString()}`);
        ...
        output.print(`Total: ${clients.length} client${clients.length === 1 ? "" : "s"}`);
    }
    
    logger.info("OAuth clients listed successfully", { count: clients.length, format });
  } catch (error: any) {
...
export async function oauthDelete(options: OAuthDeleteOptions): Promise<void> {
  ...
  try {
    logger.debug("Deleting OAuth client", { clientId: options.clientId, force: options.force });
    
    if (!options.force) {
      output.print(`WARNING: This will permanently delete OAuth client '${options.clientId}'.`);
      output.print("Any applications using this client will lose access.");
      output.print("Use --force to skip this confirmation.");
      logger.info("OAuth client deletion cancelled - confirmation required", { clientId: options.clientId });
      process.exit(0);
    }
    
    const result = await apiRequest<void>(`/api/oauth/clients/${options.clientId}`, {
      method: "DELETE",
      token: options.token,
      server: options.server,
    });
    ...
    if (!result.success) {
      if (result.error.code === "NOT_FOUND") {
        ...
        logger.warn("OAuth client not found for deletion", { clientId: options.clientId });
      } else {
        output.error("Error: " + result.error.message);
        logger.error("OAuth client deletion failed", { clientId: options.clientId, error: result.error });
      }
      process.exit(1);
    }
    
    output.success(`OAuth client '${options.clientId}' deleted successfully.`);
    logger.info("OAuth client deleted successfully", { clientId: options.clientId });
  } catch (error: any) {
    logger.error("OAuth delete command failed", { clientId: options.clientId, error: error.message });
    output.error("Error: " + error.message);
    process.exit(1);
  }
}
...
export async function oauthInfo(options: OAuthInfoOptions): Promise<void> {
  ...
  try {
    ...
    const result = await apiRequest<OAuthClient>(`/api/oauth/clients/${options.clientId}`, {
      token: options.token,
      server: options.server,
    });
    ...
    if (!result.success) {
      if (result.error.code === "NOT_FOUND") {
        output.error(`OAuth client '${options.clientId}' not found.`);
        logger.warn("OAuth client not found for info", { clientId: options.clientId });
      } else {
        output.error("Error: " + result.error.message);
        logger.error("OAuth client info failed", { clientId: options.clientId, error: result.error });
      }
      process.exit(1);
    }
    ...
    const format = options.format || "table";
    logger.debug("Displaying OAuth client info", { clientId: options.clientId, format });
    ...
    switch (format) {
      ...
      default: // table
        ...
        output.print(`Created:        ${new Date(client.created_at).toLocaleString()}`);
        output.print(`Redirect URIs:  ${client.redirect_uris.join(", ")}`);
        output.print(`Requested Pods: ${client.requested_pods.join(", ") || "None"}`);
    }
    
    logger.info("OAuth client info displayed", { clientId: options.clientId, format });
  } catch (error: any) {
    logger.error("OAuth info command failed", { clientId: options.clientId, error: error.message });
    output.error("Error: " + error.message);
    process.exit(1);
  }
}