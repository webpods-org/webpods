...
export async function configSet(options: ConfigSetOptions): Promise<void> {
  ...
  try {
    logger.debug("Setting configuration", { key: options.key, value: options.value });
    ...
    if (!validKeys.includes(options.key)) {
      ...
      logger.error("Invalid configuration key", { key: options.key, validKeys });
      process.exit(1);
    }
    
    await updateConfig(options.key as any, options.value);
    output.success(`Configuration updated: ${options.key} = ${options.value}`);
    logger.info("Configuration updated", { key: options.key, value: options.value });
  } catch (error: any) {
    logger.error("Config set command failed", { key: options.key, value: options.value, error: error.message });
    output.error("Error: " + error.message);
    process.exit(1);
  }
}
 ...
 */
export async function configServer(options: ConfigServerOptions): Promise<void> {
  const output = createCliOutput(options.quiet);
  ...
  try {
    ...
    try {
      ...
      logger.error("Invalid URL provided for server config", { url: options.url });
      process.exit(1);
    }
    ...
    logger.error("Config server command failed", { url: options.url, error: error.message });
    output.error("Error: " + error.message);
    process.exit(1);
  }
}