...
export async function createTestPod(
  ...
  await db.none(
    `INSERT INTO stream (pod_name, stream_id, user_id, access_permission, created_at, updated_at)
  ...
  await db.none(
    `INSERT INTO record (stream_pod_name, stream_id, index, content, content_type, name, hash, previous_hash, user_id, created_at)
